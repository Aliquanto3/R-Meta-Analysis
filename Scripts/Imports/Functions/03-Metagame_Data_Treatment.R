...
archetype_metrics = function(df, presence){
  ...
  metric_df$CILowerBound = mapply(FUN = function(wins, defeats){
    binom.test(wins, wins + defeats, p = 0.5, alternative = "two.sided", 
               conf.level = CIPercent)$conf.int[1] * 100
  }, metric_df$Wins, metric_df$Defeats)
  
  metric_df$CIUpperBound =  mapply(FUN = function(wins, defeats){
    binom.test(wins, wins + defeats, p = 0.5, alternative = "two.sided", 
...
#' Add the tiers to the archetype metric dataframe
#'
#' @param metric_df the dataframe generated by archetype_metrics()  
...
archetype_normalized_sum = function(metric_df, statShare){
  ...
  meanMetric = mean(metric_df_normalized$NormalizedSum)
  sdMetric = sd(metric_df_normalized$NormalizedSum)
  
  metric_df_normalized = metric_df_normalized %>% 
